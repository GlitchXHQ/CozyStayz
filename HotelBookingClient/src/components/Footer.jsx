import React from 'react'
import { assets } from '../assets/assets'
import { Link, useNavigate } from 'react-router-dom'

const Footer = () => {

    const navigate=useNavigate()

    return (
        <footer className="px-6 pt-8 md:px-16 lg:px-36 w-full text-gray-800 bg-white border-t-2 border-gray-600">
            <div className="flex flex-col md:flex-row justify-between w-full gap-10 border-b border-gray-500 pb-10">
                <div className="md:max-w-96">
                    <div className='flex flex-row items-center gap-3 text-xl font-bold'>
                        <img src={assets.guestsIcon} alt="" />
                        <p>CozyStayz</p>
                    </div>
                    <p className="mt-6 text-sm">
                        CozyStayz — Your trusted companion for comfortable stays, seamless bookings, and unforgettable travel experiences.
                    </p>
                </div>
                <div className="flex-1 flex items-start md:justify-end gap-20 md:gap-40">
                    <div className='flex flex-col'>
                        <h2 className="font-semibold mb-5">Company</h2>
                        <ul className="text-sm space-y-2 flex flex-col">
                            <button onClick={()=>navigate('/')} className='cursor-pointer'>Home</button>
                            <button onClick={()=>navigate('/About')} className='cursor-pointer'>About Us</button>
                            <button onClick={()=>navigate('/Contact')} className='cursor-pointer'>Contact Us</button>
                        </ul>
                    </div>
                    <div>
                        <h2 className="font-semibold mb-5">Get in touch</h2>
                        <div className="text-sm space-y-2">
                            <p>+1-234-567-890</p>
                            <p>CozyStayz@x.com</p>
                        </div>
                    </div>
                </div>
            </div>
            <p className="pt-4 text-center text-sm pb-5">
                Copyright {new Date().getFullYear()} © <a href="https://prebuiltui.com">CozyStayz</a>. All Right Reserved.
            </p>
        </footer>
    )
}

export default Footer